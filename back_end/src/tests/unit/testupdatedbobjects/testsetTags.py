...
class TestSetTagsHandler(TestCase):
    ...
    def testProcessesEvent(self, mockDB, mockGetVenueMethod, mockGetMenuMethod):
        self.assertEqual(self.handler.handle_request({}, {}), {'statusCode': self.handler.successCode})

    @patch('main.updatedbobjects.setTags.SetTagsHandler.getAllVenues', return_value=MOCKFAILVENUEGETRESULT)
    def testReturnsErrorCodeIfCannotGetVenues(self, mockVenueMethod):
        self.assertEqual(self.failCase, self.handler.handle_request({}, {}))